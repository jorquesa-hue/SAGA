-- Milestone 4: parcela generation engine. Spec §4: "Parcela é gerada na
-- assinatura do contrato, não sob demanda." Called once, right after the
-- app sets contratos.assinado_em — never regenerated afterwards (guarded
-- below).
--
-- SECURITY INVOKER (default): runs as the calling `authenticated` role, so
-- the INSERT into parcelas is still checked against parcelas_insert_staff
-- (admin/secretaria, own escola only) exactly like any other client write
-- — no privilege escalation is granted by wrapping this in a function.
...
begin
  ...
  for i in 1..v_contrato.num_parcelas loop
    ...
    v_ultimo_dia_mes := extract(day from (date_trunc('month', v_competencia) + interval '1 month - 1 day'))::int;
    v_dia := least(v_contrato.vencimento_dia, v_ultimo_dia_mes);
    v_vencimento := make_date(extract(year from v_competencia)::int, extract(month from v_competencia)::int, v_dia);
...
grant execute on function fn_gerar_parcelas(uuid) to authenticated;
