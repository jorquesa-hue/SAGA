...
describe.skipIf(!available)("AssetsMaintenanceService (integration)", () => {
  ...
  beforeAll(async () => {
    ...
    const seeded = await seedTenantWithOwner(identity, "Fazenda Ativos", "owner@example.com");
  ...
  it("registers an asset and schedules calibration", async () => {
    ...
    const sched = await assets.defineSchedule(owner, { assetId: scale.id, kind: "calibration", intervalDays: 180 });
    expect(sched.nextDueAt).toBeTruthy();
  });

  it("tracks calibration status (valid vs expired) (JK-AST-004, JK-WGT-007)", async () => {
    const scale = await assets.registerAsset(owner, { name: "Balança 2", assetType: "scale" });
    let status = await assets.getCalibrationStatus(owner, scale.id);
    expect(status.valid).toBe(false); // never calibrated

    await assets.recordCalibration(owner, scale.id, new Date(Date.now() + 90 * 86400000).toISOString());
    status = await assets.getCalibrationStatus(owner, scale.id);
    expect(status.valid).toBe(true);

    await assets.recordCalibration(owner, scale.id, new Date(Date.now() - 86400000).toISOString());
    status = await assets.getCalibrationStatus(owner, scale.id);
    expect(status.valid).toBe(false); // expired
  });

  it("opens and completes a work order, toggling asset status", async () => {
    const pump = await assets.registerAsset(owner, { name: "Bomba 1", assetType: "pump" });
    const wo = await assets.createWorkOrder(owner, { assetId: pump.id, description: "Troca de rolamento", priority: "high" });

    let a = await db.adminPool.query(`SELECT status FROM asset WHERE id = $1`, [pump.id]);
    expect(a.rows[0].status).toBe("maintenance");

    await assets.completeWorkOrder(owner, wo.workOrderId, { laborCost: 150, partsCost: 300, downtimeHours: 4 });
    a = await db.adminPool.query(`SELECT status FROM asset WHERE id = $1`, [pump.id]);
    expect(a.rows[0].status).toBe("active");

    const closed = await db.adminPool.query(`SELECT status, parts_cost FROM work_order WHERE id = $1`, [wo.workOrderId]);
    expect(closed.rows[0].status).toBe("done");
    expect(Number(closed.rows[0].parts_cost)).toBe(300);
  });

  it("lists due maintenance", async () => {
    const asset = await assets.registerAsset(owner, { name: "Trator", assetType: "machinery" });
  ...
  it("rejects scheduling on a missing asset", async () => {
    await expect(
      assets.defineSchedule(owner, { assetId: newUuid(), kind: "preventive", intervalDays: 30 }),
    ).rejects.toBeInstanceOf(NotFoundError);
  });

  it("does not leak assets across tenants", async () => {
    const asset = await assets.registerAsset(owner, { name: "Secreto", assetType: "gateway" });
    const other = await seedTenantWithOwner(identity, "Outra", "o@example.com");
    await expect(assets.getCalibrationStatus(other.ownerContext, asset.id)).rejects.toBeInstanceOf(NotFoundError);
  });
});
