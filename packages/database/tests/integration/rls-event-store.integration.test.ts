...
describe.skipIf(!available)("RLS tenant isolation + event store (real PostgreSQL)", () => {
  let db: TestDatabase;
  let tenantA: string;
  let tenantB: string;
  ...
  it("app role sees only the active tenant's rows (JK-SEC-004/005)", async () => {
    await withTenantTransaction(db.appPool, contextFor(tenantA), async (client) => {
      await client.query(`INSERT INTO farm (tenant_id, name, area_ha) VALUES ($1, 'Sede', 100)`, [
        tenantA,
      ]);
    });
  ...
  it("blocks cross-tenant INSERT even when application code lies (RLS WITH CHECK)", async () => {
    await expect(
      withTenantTransaction(db.appPool, contextFor(tenantB), (client) =>
        client.query(`INSERT INTO farm (tenant_id, name) VALUES ($1, 'Invasora')`, [tenantA]),
      ),
  ...
  it("appends events atomically with the outbox and replays idempotently", async () => {
    ...
    const count = await withTenantTransaction(db.appPool, context, (client) =>
      client.query(`SELECT count(*)::int AS n FROM domain_event WHERE aggregate_id = $1`, [
        aggregateId,
      ]),
    );
  ...
  it("worker role reads the outbox across tenants but app role cannot", async () => {
    ...
    const fromB = await withTenantTransaction(db.appPool, contextFor(tenantB), (client) =>
      client.query("SELECT tenant_id FROM outbox_message"),
    );
