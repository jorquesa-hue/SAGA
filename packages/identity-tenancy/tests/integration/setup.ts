...
/**
 * Disposable-database integration harness (same pattern as
 * packages/database/tests/integration/setup.ts): creates a scratch database,
 * provisions the local application roles, applies all migrations, and exposes
 * pools for the admin (owner/system), app (RLS-enforced), and worker roles.
 */
...
export async function createTestDatabase(prefix = "jk_idt"): Promise<TestDatabase> {
