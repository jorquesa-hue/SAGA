...
export class AnimalRegistryService {
  ...
  async registerAnimal(
    ...
    return this.authorized(
      ...
      async (client) => {
        ...
        const farm = await client.query(`SELECT 1 FROM farm WHERE id = $1`, [
          input.farmId,
        ]);
  ...
  private async bumpAnimalVersion(client: pg.PoolClient, animalId: Uuid): Promise<void> {
    await client.query(`UPDATE animal SET version = version + 1 WHERE id = $1`, [
      animalId,
    ]);
  }
