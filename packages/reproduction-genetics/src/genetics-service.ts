...
export class GeneticsService {
  ...
  async importEvaluation(context: TenantContext, rawInput: ImportEvaluationInput): Promise<{ evaluationId: Uuid }> {
    const input = this.parse(importEvaluationInputSchema, rawInput);
    return this.authorized(context, true, async (client) => {
      const animal = await client.query(`SELECT 1 FROM animal WHERE id = $1`, [input.animalId]);
      if (animal.rows.length === 0) throw new NotFoundError(`Animal ${input.animalId} not found`);
      const id = newUuid();
      let eventId: string;
      try {
        const env = createEventEnvelope({
          ...
          aggregateVersion: await this.nextVersion(client, context.tenantId, input.animalId),
          source: { channel: "import" },
          idempotencyKey: input.idempotencyKey ?? `eval-${id}`,
          payload: { evaluationId: id, animalId: input.animalId, provider: input.provider, trait: input.trait, value: input.value },
        });
        eventId = (await appendEvent(client, env, { environment: this.environment })).eventId;
        await client.query(
          ...
          [
            id, context.tenantId, input.animalId, input.provider, input.evaluationDate,
            input.trait, input.value, input.percentile ?? null, input.reliability ?? null,
            input.sourceFile ?? null, eventId,
          ],
        );
      } catch (error) {
        if ((error as { code?: string }).code === "23505") {
          throw new ValidationError("This evaluation (animal/provider/date/trait) was already imported");
        }
        throw error;
      }
  ...
  async defineSelectionIndex(context: TenantContext, rawInput: DefineIndexInput): Promise<{ indexId: Uuid }> {
    ...
    return this.authorized(context, true, async (client) => {
      try {
        const inserted = await client.query(
          ...
          [context.tenantId, input.name, input.version, JSON.stringify(input.weights), input.missingDataBehavior],
        );
        return { indexId: inserted.rows[0]!.id };
      } catch (error) {
        if ((error as { code?: string }).code === "23505") {
          throw new ValidationError(`Selection index '${input.name}' version ${input.version} already exists`);
        }
        throw error;
      }
    });
   ...
   */
  async rankAnimals(context: TenantContext, indexId: Uuid, animalIds: Uuid[]): Promise<RankedAnimal[]> {
    ...
    return this.authorized(context, false, async (client) => {
      const idx = await client.query<{ weights: Record<string, number>; missing_data_behavior: string }>(
        `SELECT weights, missing_data_behavior FROM selection_index WHERE id = $1`,
        [indexId],
      );
      if (idx.rows.length === 0) throw new NotFoundError(`Selection index ${indexId} not found`);
      ...
      const evals = await client.query<{ animal_id: string; trait: string; value: string; percentile: string | null }>(
      ...
      const byAnimalTrait = new Map<string, { value: number; percentile: number | null }>();
      ...
      for (const t of traits) {
        ...
        if (vals.length > 0) range.set(t, { min: Math.min(...vals), max: Math.max(...vals) });
      }
      ...
      const ranked: RankedAnimal[] = animalIds.map((animalId) => {
        ...
        for (const t of traits) {
          ...
          if (cell === undefined) {
            if (missing === "exclude") {
              excluded = true;
              breakdown.push({ trait: t, rawValue: null, normalized: null, weight, contribution: 0, percentile: null });
              continue;
            }
            breakdown.push({ trait: t, rawValue: 0, normalized: 0, weight, contribution: 0, percentile: null });
            continue;
          }
          const normalized = r && r.max > r.min ? (cell.value - r.min) / (r.max - r.min) : 0.5;
          const contribution = weight * normalized;
          score += contribution;
          breakdown.push({ trait: t, rawValue: cell.value, normalized, weight, contribution, percentile: cell.percentile });
        }
        return excluded
          ? { animalId, score: null, excluded: true, exclusionReason: "missing required trait (index excludes incomplete data)", breakdown }
          : { animalId, score: Math.round(score * 10000) / 10000, excluded: false, breakdown };
      });
  ...
  async geneticProgress(context: TenantContext, trait: string): Promise<Array<{ birthYear: number; avgValue: number; count: number }>> {
    return this.authorized(context, false, async (client) => {
      const result = await client.query<{ birth_year: number; avg_value: string; n: string }>(
      ...
      return result.rows.map((r) => ({ birthYear: r.birth_year, avgValue: Number(r.avg_value), count: Number(r.n) }));
    });
  }

  // -- internals --
  private async nextVersion(client: pg.PoolClient, tenantId: string, animalId: string): Promise<number> {
  ...
  private async authorized<T>(
    ...
    const outcome = await withTenantTransaction(this.appPool, context, async (client) => {
      ...
      if (write && !active.some((m) => GENETICS_WRITE_ROLES.has(m.role))) {
        return { ok: false as const, reason: "genetics changes require genetics_specialist or tenant_owner" };
      }
