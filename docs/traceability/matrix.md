...
| JK-ANI-009 multi-species registration (bovine/porcine/ovine/caprine/equine) | species catalog | `animal.species_code` CHECK (migration 0022); `speciesCodeSchema`; web species selector | `validation.test.ts`, `animal-registry.integration.test.ts` (non-bovine registration), `animals.test.tsx` | implemented |
| JK-ANI-010 dated photo gallery per animal (not a single profile photo) | photo gallery | `animal_photo` table (migration 0022, soft-status); `addPhotoMetadata`/`removePhoto`/`listPhotos`; `POST/GET/DELETE /animals/:id/photos`; S3-compatible storage client (`apps/api/src/storage.ts`) | `animal-registry.integration.test.ts` (gallery + soft-remove + tenant isolation), `animal-photos.integration.test.ts` (multipart upload/list/download/remove), `animal-photos-page.test.tsx` | implemented (MinIO live-storage path unverified in this environment — see blockers) |
