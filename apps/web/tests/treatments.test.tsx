...
function capturingClient(): { client: JkPlatformClient; posts: { path: string; body: unknown }[] } {
  const posts: { path: string; body: unknown }[] = [];
  const fetch: FetchLike = async (url, init) => {
    const path = url.replace(/^https?:\/\/[^/]+/, "").replace(/\?.*$/, "");
    if ((init?.method ?? "GET") === "POST") posts.push({ path, body: JSON.parse(init?.body ?? "{}") });
    return { status: 201, headers: { get: () => "c" }, text: async () => JSON.stringify({ id: "x1" }) };
  };
  return { client: new JkPlatformClient({ baseUrl: "http://api.test", tenantId: "t", auth: { mode: "none" }, fetch }), posts };
}
...
describe("Treatments command", () => {
  it("posts a treatment with a withdrawal restriction hint", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("animal uuid"), { target: { value: "a-1" } });
    fireEvent.change(screen.getByPlaceholderText("ex.: Ivermectina"), { target: { value: "Ivermectina" } });
    fireEvent.click(screen.getByText("Registrar"));
    await waitFor(() => expect(screen.getByText(/Tratamento registrado/)).toBeInTheDocument());
...
describe("Finance command", () => {
  it("routes an expense vs a sale to the correct endpoints", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("ex.: nutrição"), { target: { value: "nutrição" } });
    fireEvent.change(screen.getByLabelText("Valor (ex.: 1250.00)"), { target: { value: "1250.00" } });
    fireEvent.click(screen.getByText("Registrar"));
    await waitFor(() => expect(posts.some((p) => p.path === "/api/v1/finance/expenses")).toBe(true));
  });
});
