...
describe.skipIf(!available)("End-to-end farm scenario (integration)", () => {
  ...
  const read = (userId: string, tenantId: string) => ({
    "x-dev-user-id": userId,
  ...
  it("registers → treats → withdrawal blocks sale → exports packet → search finds it", async () => {
    ...
    const tenantRes = await app.inject({
      ...
      payload: {
        name: "Fazenda E2E",
        owner: { email: "dono@example.com", displayName: "Dono" },
      },
    });
    ...
    const farmRes = await app.inject({
      ...
      payload: { name: "Sede E2E", areaHa: 120 },
    });
    ...
    const clearRes = await app.inject({
      method: "GET",
      url: `/api/v1/animals/${animalId}/sale-clear`,
      headers: read(ownerId, tenantId),
    });
    ...
    const restrRes = await app.inject({
      method: "GET",
      url: `/api/v1/animals/${animalId}/restrictions`,
      headers: read(ownerId, tenantId),
    });
    expect(
      restrRes
        .json()
        .items.some(
          (r: { restrictionType: string }) => r.restrictionType === "withdrawal",
        ),
    ).toBe(true);
    ...
    const processRes = await app.inject({
      method: "POST",
      url: `/api/v1/exports/${exportId}/process`,
      headers: cmd(ownerId, tenantId),
    });
    ...
    expect(
      packet.restrictions.some(
        (r: { restriction_type: string }) => r.restriction_type === "withdrawal",
      ),
    ).toBe(true);
    expect(
      packet.identifiers.some(
        (i: { identifier_value: string }) => i.identifier_value === "982000000E2E001",
      ),
    ).toBe(true);
    ...
    const searchRes = await app.inject({
      method: "GET",
      url: "/api/v1/search?q=BR-E2E",
      headers: read(ownerId, tenantId),
    });
    expect(searchRes.statusCode).toBe(200);
    expect(
      searchRes.json().animals.some((a: { label: string }) => a.label === "BR-E2E-1"),
    ).toBe(true);

    const rfidSearch = await app.inject({
      method: "GET",
      url: "/api/v1/search?q=982000000E2E001",
      headers: read(ownerId, tenantId),
    });
  ...
  it("enforces the tenant base currency on finance writes and reports margin in it", async () => {
    ...
    const tenantRes = await app.inject({
      ...
      payload: {
        name: "Rancho USD",
        defaultCurrency: "USD",
        owner: { email: "usd@example.com", displayName: "USD Owner" },
      },
    });
    expect(tenantRes.statusCode).toBe(201);
    const tenantId = tenantRes.json().tenant.id as string;
    ...
    const marginRes = await app.inject({
      method: "GET",
      url: `/api/v1/lots/${newUuid()}/margin`,
      headers: read(ownerId, tenantId),
    });
