...
export function registerReproductionRoutes(app: FastifyInstance, service: ReproductionGeneticsService): void {
  const ctx = (r: FastifyRequest) => buildTenantContext(r.principal, tenant(r), r.correlationId);

  app.post("/api/v1/reproduction/services", async (r: FastifyRequest, reply: FastifyReply) => {
    const key = idem(r);
    const body = (r.body ?? {}) as object;
    const svc = await service.recordService(ctx(r), { ...body, idempotencyKey: `service:${key}` } as never);
    reply.status(201);
    return svc;
  });

  app.post("/api/v1/reproduction/pregnancy-checks", async (r: FastifyRequest, reply: FastifyReply) => {
    const key = idem(r);
    const body = (r.body ?? {}) as object;
    const check = await service.recordPregnancyCheck(ctx(r), { ...body, idempotencyKey: `pregcheck:${key}` } as never);
    reply.status(201);
    return check;
  });
  ...
  app.post("/api/v1/calvings", async (r: FastifyRequest, reply: FastifyReply) => {
    const key = idem(r);
    const body = (r.body ?? {}) as object;
    const calving = await service.recordCalving(ctx(r), { ...body, idempotencyKey: `calving:${key}` } as never);
    reply.status(201);
    return calving;
  });
